...
Data mining it also being used to predict the response of a user to an online
ad\cite{chapelle2013simple}. Their main object is to develop a framework that
can predict the result of an user clicking in an advertising will be based on
the history that they have.
The proposed framework use Maximum Entropy\cite{Nigam99usingmaximum}
because it is easy to implement, can be parallelized and can scale with respect
...
In another approach to same problem improvements in the context of traditional
supervised learning based on FTRL-proximal online learning algorithm are
introduced\cite{McMahan:2013:ACP:2487575.2488200}. This paper also explores ways
...
the solve the memory problem they also test encoding values with fewer bits and
other techniques.

In yet another paper about CTR prediction
\cite{Tagami:2013:CPC:2501040.2501978}, a two stage approach is used, the first
stage the contruction of a ranking model with the clicked ad requests and then a
sigmoid function converts the value of the ranking model into CTR. The method
